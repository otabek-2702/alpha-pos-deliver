      ...
      <div className="pay__body">
        ...
        {val > 0 && (
          <div className={"change-banner " + (diff >= 0 ? "ok" : "short")}>
            ...
            <span className="change-banner__v mono is-tween" key={diff}>{fmt.money(Math.abs(diff))}</span>
          </div>
        )}
  ...
  return (
    <div className="pay">
      ...
      <div className="pay__body">
        ...
        <div className="qrcard is-waiting">
          {!reduce && <div className="qr-shimmer"></div>}
...
function PaidView({ amount, method, order, splitCash, T, onDone }) {
  ...
  return (
    <div className="pay pay--paid">
      ...
      <div className="pay__body" style={{ justifyContent: "flex-start" }}>
        <div className="paid-burst">
          <div className="paid-burst__ring" style={{ position: "relative" }}>
            {!reduce && <div className="ripple" style={{ borderColor: "#fff" }}></div>}
            <DrawCheck size={58} stroke="var(--success-strong)" pop />
          </div>
        </div>
