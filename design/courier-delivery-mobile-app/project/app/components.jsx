...
function Badge({ tone, dot, children }) {
  return (
    <span className={"badge t-" + tone + (dot ? " badge--dot" : "")}>{children}</span>
  );
}
...
// ---- bottom tab bar ----
function TabBar({ active, onChange, unread, T }) {
  const tr = T || ((k) => k);
  const tabs = [
...
Object.assign(window, {
  stepBadge, Badge, nextAction, MapView, NavSheet, NAV_APPS, PushBanner, TabBar,
});
